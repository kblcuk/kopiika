...
- `build-transactions.ts` — turns every selected row plus its assignment into `Transaction[]` (and any newly-created category entities, each paired with the optional planned amount the user entered) for commit through the normal `createTransactionBatch` path, so every built transaction still runs through `validateTransaction` and a single violation aborts the whole batch. Selection — not status — decides what commits: `new` rows are selected by default, and a `duplicate` the user deliberately ticks imports too (see below).

Kopiika has no free-floating amounts — every transaction moves between two entities — so a bank row's sign selects the counterparty type against the account already in context: a negative row (outflow) becomes `Account → Category`, a positive row (inflow) becomes `Income → Account`. The user can instead mark either direction as a transfer, producing `Account → Account` (outflow: account → other account; inflow: other account → account); a light auto-suggestion pre-selects a transfer counterparty when the row's description contains another account's name. New categories are created inline through the same `EntityCreateModal` used everywhere else (in its staging mode — it emits an `EntityDraft` of name/icon/color/planned-amount rather than writing to the store), so an import-time category gets the full picker and an optional plan, all committed atomically with the transactions. A category staged this way is derived back out of the rows' `newCategory` assignments (keyed by trimmed name) and injected into the category picker as a pseudo-entity, so later rows can reuse it without it going stale; selecting it just points the row at the same pending category, and dedup-by-name in `build-transactions.ts` collapses them to one committed entity. `Account → Saving` is intentionally not offered here — savings use the distinct additive reservation UX described above. Imported rows adopt the account's currency, the bank description becomes the transaction `note`, and rows are historical (`is_confirmed = true`).
...
Split transactions get folded before matching. A split is stored as N independent rows sharing a `split_id` (see `buildSplitRows`) — one per category, with no parent row — yet the bank only ever reports the original total. So `reconcile.ts` sums each `split_id` group's account-effect into a single entry keyed by (total, civil date); a statement line then matches the reconstructed total, and the individual legs are no longer matchable, which is correct. Rows created before KII-146 were stamped by migration `0024`, which applied the timestamp+note heuristic `reconcile` previously ran inline; groups it declined (a repeated category, or a coincidental timestamp+note collision) stay unstamped and their total surfaces as `new` — a reviewable extra row, never a false positive, matching reconciliation's "when uncertain, prefer `new`" stance. The backfill errs in the other direction too: two genuinely-separate charges that happened to share an account, timestamp and note while carrying distinct categories are fused into one group, and their totals then fold into a single entry that can mask a real statement line as a duplicate. Unlike the old per-read inference, which re-guessed on every import and could be undone by editing a row, that mis-fold is now persistent — `split_id` survives edits (`updateTransaction` writes only the keys it is given) and no UI surface clears it; the class is at least bounded, since only rows present at migration `0024` were ever stamped by the heuristic and rows imported afterwards never are. Conversely, a genuinely-new charge that happens to equal a real split's total on the same civil day now folds-and-matches and is flagged a deselected duplicate; this is not new behavior peculiar to splits but the same day+amount collision that already applies to unsplit transactions — deselected, not dropped, and visible in review via the "Already have" toggle. The match is intentionally conservative — a near-miss (e.g. posting-date drift) surfaces as `new` rather than being silently hidden, and the review step makes that visible. A duplicate defaults to unselected and shows an "Already have" badge, but the user can still tick it to import anyway — doing so reveals the same category picker as a new row, and (like any selected row) it must be assigned before commit is enabled. To keep a long statement scannable, each review row carries a state cue driven purely by selection + assignment: a row that will import but still needs a category gets an amber left edge and a faint amber wash, a row that won't import is dimmed, and a ready (selected + assigned) row stays plain — so the amber rows are exactly the remaining to-dos. A constant-width transparent left border reserves the stripe's space in every state so toggling a row never shifts its content.
