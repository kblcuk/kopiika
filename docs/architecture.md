...
Split transactions get folded before matching. A split is stored as N independent rows (no parent/total; see `buildSplitRows`) sharing an identical `timestamp` and `note` but distributing one charge across distinct categories — yet the bank only ever reports the original total. So `reconcile.ts` groups existing account rows by `timestamp | note` and, when a group is a genuine split (≥2 same-direction rows with pairwise-distinct counterparty categories), sums their account-effect into a single entry keyed by (total, civil date). A statement line then matches the reconstructed total; the individual legs are no longer matchable, which is correct. The distinct-category guard is load-bearing: since `parseFlexibleDate` drops the time component, every imported row lands on the same midnight timestamp with `note = description`, so two legitimately-separate identical charges would otherwise be mis-folded and break dedup on re-import; requiring distinct categories tells a real split apart from duplicate charges. The guard is deliberately conservative, so a few real splits aren't folded and their total surfaces as `new` instead: a split with a _repeated_ category (set smaller than leg count — includes the fully same-category degenerate case), and a split that coincidentally shares timestamp+note with an unrelated charge. Both are safe — a reviewable extra row, never a false positive — matching reconciliation's "when uncertain, prefer `new`" stance. Conversely, a genuinely-new charge that happens to equal a real split's total on the same civil day now folds-and-matches and is flagged a deselected duplicate; this is not new behavior peculiar to splits but the same day+amount collision that already applies to unsplit transactions — deselected, not dropped, and visible in review via the "Already have" toggle. The match is intentionally conservative — a near-miss (e.g. posting-date drift) surfaces as `new` rather than being silently hidden, and the review step makes that visible. A duplicate defaults to unselected and shows an "Already have" badge, but the user can still tick it to import anyway — doing so reveals the same category picker as a new row, and (like any selected row) it must be assigned before commit is enabled. To keep a long statement scannable, each review row carries a state cue driven purely by selection + assignment: a row that will import but still needs a category gets an amber left edge and a faint amber wash, a row that won't import is dimmed, and a ready (selected + assigned) row stays plain — so the amber rows are exactly the remaining to-dos. A constant-width transparent left border reserves the stripe's space in every state so toggling a row never shifts its content.
