...
export function getReservationForPair(
  ...
  for (const tx of transactions) {
    if (tx.from_entity_id === accountId && tx.to_entity_id === savingId) {
      ...
      tx.from_entity_id === savingId &&
      tx.to_entity_id === accountId
    ) {
      net -= tx.amount;
    }
  }
...
export function getReservationsForSaving(
  ...
  const accountIds = new Set(
    entities.filter((e) => e.type === "account").map((e) => e.id),
  );
  ...
  for (const tx of transactions) {
    if (tx.to_entity_id === savingId && accountIds.has(tx.from_entity_id)) {
      perAccount.set(
        tx.from_entity_id,
        (perAccount.get(tx.from_entity_id) ?? 0) + tx.amount,
      );
    } else if (
      tx.from_entity_id === savingId &&
      accountIds.has(tx.to_entity_id)
    ) {
      perAccount.set(
        tx.to_entity_id,
        (perAccount.get(tx.to_entity_id) ?? 0) - tx.amount,
      );
    }
  }
...
export function getReservationsForAccount(
  ...
  const savingIds = new Set(
    entities.filter((e) => e.type === "saving").map((e) => e.id),
  );
  ...
  for (const tx of transactions) {
    if (tx.from_entity_id === accountId && savingIds.has(tx.to_entity_id)) {
      perSaving.set(
        tx.to_entity_id,
        (perSaving.get(tx.to_entity_id) ?? 0) + tx.amount,
      );
    } else if (
      tx.to_entity_id === accountId &&
      savingIds.has(tx.from_entity_id)
    ) {
      perSaving.set(
        tx.from_entity_id,
        (perSaving.get(tx.from_entity_id) ?? 0) - tx.amount,
      );
    }
  }
